...
if (isset($_POST['action'])) {
    ...
    switch ($_POST['action']) {
        ...
        case 'get_album_photos':
            $album_id = $_POST['id'];
            $album = fetchRow("SELECT pa.*, e.title as event_title, b.year as batch_year, b.semester as batch_semester FROM photo_albums pa LEFT JOIN events e ON pa.event_id = e.id LEFT JOIN batches b ON pa.batch_id = b.id WHERE pa.id = ?", [$album_id]);
            $photos = fetchAll("SELECT p.*, pa.title as album_title FROM photos p JOIN photo_albums pa ON p.album_id = pa.id WHERE p.album_id = ? ORDER BY p.upload_date DESC", [$album_id]);
            echo json_encode(['album' => $album, 'photos' => $photos]);
            exit;
...
// Get albums with photo count and first photo
$albums = fetchAll("
    SELECT pa.*, COUNT(p.id) as photo_count, a.full_name as admin_name,
           ...
           (SELECT file_path FROM photos WHERE album_id = pa.id ORDER BY upload_date ASC LIMIT 1) as first_photo
    FROM photo_albums pa 
...
<head>
    ...
    <title>Photo Gallery - SCI Alumni System</title>
...
<?php include("include/navbar.php"); ?>            
            ...
            <div class="container">
                <div class="page-inner">
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        ...
                                        <div class="col-md-4 mb-4">
                                            <div class="card">
                                                <div class="card-img-top" style="height: 200px; background: #f8f9fa; display: flex; align-items: center; justify-content: center; position: relative; overflow: hidden;">
                                                    <?php if ($album['first_photo'] && file_exists($album['first_photo'])): ?>
                                                        <img src="<?= htmlspecialchars($album['first_photo']) ?>" alt="<?= htmlspecialchars($album['title']) ?>" style="width: 100%; height: 100%; object-fit: cover;">
                                                    <?php else: ?>
                                                ...
                                                <div class="card-body text-center">
                                                    ...
                                                    <div class="btn-group" role="group">
                                                        <button type="button" class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#albumViewModal" onclick="viewAlbumPhotos(<?= $album['id'] ?>)">
                    ...
                    <!-- Removed: Static album photos display section - now using modal instead -->
                </div>
            </div>
...
<?php include("include/footer.php"); ?>
                ...
                <form id="photoForm" enctype="multipart/form-data">
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="location" placeholder="e.g., SCI Main Campus">
                        </div>
                ...
                <div class="modal-body">
                    <div class="row g-0">
                        <div class="col-md-8 text-center border-end">
                            <img id="photoPreviewImage" class="img-fluid rounded" src="" alt="Photo preview">
                        </div>
                        <div class="col-md-4 ps-md-4 mt-3 mt-md-0">
                            <h5 id="photoPreviewTitle" class="fw-bold"></h5>
                            <p class="text-muted small mb-1" id="photoPreviewFileName"></p>
                            <p id="photoPreviewDescription" class="small text-body-secondary mb-3"></p>
                            <p class="photo-preview-meta" id="photoPreviewMeta"></p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <!-- Album View Modal -->
    <div class="modal fade" id="albumViewModal" tabindex="-1" aria-labelledby="albumViewModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="albumViewModalLabel">Album Photos</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div id="albumViewContent">
    ...
    <script>
        ...
        const photoPreviewModalInstance = photoPreviewModalElement ? new bootstrap.Modal(photoPreviewModalElement) : null;
        
        // When photo preview modal is closed, reopen album view modal if it was open
        if (photoPreviewModalElement) {
            photoPreviewModalElement.addEventListener('hidden.bs.modal', function() {
                const albumViewModal = document.getElementById('albumViewModal');
                if (albumViewModal && albumViewModal.classList.contains('show')) {
                    // Modal is already shown, no need to reopen
                } else if (currentAlbumId) {
                    // Optionally reopen the album view modal
                }
            });
        }
        ...
        function viewAlbumPhotos(albumId) {
            ...
            albumViewContent.innerHTML = '<div class="text-center py-5"><div class="spinner-border" role="status"><span class="visually-hidden">Loading...</span></div></div>';
            
            // Fetch album photos via AJAX
            $.post('manage_photos.php', {action: 'get_album_photos', id: albumId}, function(response) {
                if (response && response.album && response.photos) {
                    let html = '<div class="album-info mb-4">';
                    html += '<h5 class="fw-bold mb-2">' + htmlEscape(response.album.title) + '</h5>';
                    if (response.album.description) {
                        html += '<p class="text-muted mb-2">' + htmlEscape(response.album.description) + '</p>';
                    }
                    html += '<p class="small text-muted mb-0">';
                    if (response.photos.length > 0) {
                        html += response.photos.length + ' photo' + (response.photos.length !== 1 ? 's' : '');
                    } else {
                        html += 'No photos in this album';
                    }
                    html += '</p>';
                    html += '</div>';
                    ...
                    if (response.photos.length > 0) {
                        ...
                        response.photos.forEach(function(photo) {
                            const photoDetails = {
                                ...
                                upload_date: photo.upload_date ? new Date(photo.upload_date).toLocaleDateString('en-US', {year: 'numeric', month: 'long', day: 'numeric'}) : '',
                                album_title: photo.album_title || ''
                            };
                            
                            html += '<div class="col-md-3 col-sm-4 col-6">';
                            html += '<div class="card h-100 shadow-sm hover-lift" style="cursor: pointer;">';
                            if (photo.file_path) {
                                html += '<div class="position-relative" style="overflow: hidden; height: 200px;">';
                                html += '<img src="' + htmlEscape(photo.file_path) + '" class="card-img-top w-100 h-100" alt="' + htmlEscape(photo.title || photo.original_filename) + '" style="object-fit: cover;">';
                                html += '</div>';
                            } else {
                                html += '<div class="card-img-top" style="height: 200px; background: #f8f9fa; display: flex; align-items: center; justify-content: center;"><i class="fas fa-image fa-3x text-muted"></i></div>';
                            }
                            html += '<div class="card-body py-2 d-flex flex-column">';
                            html += '<button class="btn btn-link text-start p-0 text-decoration-none mb-2" style="flex: 1;" onclick="showPhotoPreviewFromAlbum(' + JSON.stringify(photoDetails).replace(/"/g, '&quot;') + ')" title="' + htmlEscape(photo.title || photo.original_filename) + '">';
                            html += '<small class="d-block text-truncate fw-500">' + htmlEscape(photo.title || photo.filename) + '</small>';
                            html += '</button>';
                            html += '<div class="mt-auto">';
                            html += '<button class="btn btn-sm btn-danger w-100" onclick="deletePhoto(' + photo.id + ')" title="Delete photo"><i class="fa fa-trash me-1"></i>Delete</button>';
                            html += '</div>';
                            html += '</div>';
                            html += '</div>';
                            html += '</div>';
                        ...
                        html += '<div class="alert alert-info">No photos uploaded yet</div>';
                    }
                    
                    albumViewContent.innerHTML = html;
                } else {
                    albumViewContent.innerHTML = '<div class="alert alert-warning">Unable to load album photos</div>';
                }
            }, 'json').fail(function() {
                albumViewContent.innerHTML = '<div class="alert alert-danger">Error loading album photos</div>';
            });
        }
        
        function showPhotoPreviewFromAlbum(photo) {
            ...
            // Hide album view modal
            const albumViewModal = document.getElementById('albumViewModal');
            if (albumViewModal && albumViewModal.classList.contains('show')) {
                const instance = bootstrap.Modal.getInstance(albumViewModal) || new bootstrap.Modal(albumViewModal);
                instance.hide();
                setTimeout(() => {
        ...
        function showPhotoPreview(photo) {
            ...
            const fileEl = document.getElementById('photoPreviewFileName');
            const descriptionEl = document.getElementById('photoPreviewDescription');
            const metaEl = document.getElementById('photoPreviewMeta');

            previewImage.src = photo.path;
            previewImage.alt = photo.title || photo.original_filename || 'Photo preview';
            titleEl.textContent = photo.title || photo.original_filename || 'Untitled photo';
            fileEl.textContent = photo.original_filename || '';
            descriptionEl.textContent = photo.description || photo.caption || 'No description provided.';

            const metaParts = [];
            if (photo.album_title) {
                metaParts.push('Album: ' + photo.album_title);
            }
            if (photo.location) {
                metaParts.push('Location: ' + photo.location);
            }
            if (photo.upload_date) {
                metaParts.push('Uploaded: ' + photo.upload_date);
            }
            if (photo.caption) {
                metaParts.push('Caption: ' + photo.caption);
            }
